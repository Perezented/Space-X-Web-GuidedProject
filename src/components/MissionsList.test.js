...
test('Render list of missions after API call', () => {
    //Render the component with an empty array for the missions props
    //re-render the compoennt with missions data (simulatin the user experience of clickin the 'Get Data Button')
    const { rerender, debug, getByText, queryAllByTestId } = render(
        <MissionList missions={[]} />
    );
    expect(queryAllByTestId(/missions/i)).toHaveLength(0);

    // debug();
    rerender(<MissionList missions={missionsData} />);
    // debug();
    // query for the missions being rendered
    // assert that they are listed in the DOM
